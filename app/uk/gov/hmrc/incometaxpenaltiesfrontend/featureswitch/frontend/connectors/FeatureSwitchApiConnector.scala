...
package uk.gov.hmrc.incometaxpenaltiesfrontend.featureswitch.frontend.connectors
...
class FeatureSwitchApiConnector @Inject()(httpClient: HttpClientV2)(implicit ec: ExecutionContext) {
  ...
  def retrieveFeatureSwitches(featureSwitchProviderUrl: String
    ...
    httpClient.get(new URL(featureSwitchProviderUrl)).execute.map(
      response =>
  ...
  def updateFeatureSwitches(featureSwitchProviderUrl: String,
    ...
    httpClient
      .post(new URL(featureSwitchProviderUrl))
      .withBody(Json.toJson(featureSwitchSettings)).execute.map {
        response =>
          response.status match {
            case OK =>
              response.json.validate[Seq[FeatureSwitchSetting]] match {
