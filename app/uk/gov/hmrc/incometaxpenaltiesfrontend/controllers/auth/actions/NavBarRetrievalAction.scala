...
class NavBarRetrievalAction @Inject()(val messageCountConnector: MessageCountConnector,
  ...
  override def refine[A](request: CurrentUserRequest[A]): Future[Either[Result, CurrentUserRequest[A]]] = {
    ...
    request match {
      case _req: AuthorisedAndEnrolledIndividual[A] => request.session.get(IncomeTaxSessionKeys.origin) match {
        case Some("PTA") => handlePtaNavBar(_req)
        case Some("BTA") => handleBtaNavBar()(_req, hc)
        case _ =>
  ...
  private def handlePtaNavBar[A](req: AuthorisedAndEnrolledIndividual[A])(implicit hc: HeaderCarrier): Future[Either[Result, CurrentUserRequest[A]]] = {
    implicit val messages: Messages = messagesApi.preferred(req)
    messageCountConnector.getMessageCount().map { _ =>
      Right(req.addServiceNavigation(createPtaServiceNavigation))
    }
  }
