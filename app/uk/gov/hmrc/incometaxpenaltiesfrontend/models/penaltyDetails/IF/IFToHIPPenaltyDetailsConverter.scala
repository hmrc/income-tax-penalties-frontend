...
object IFToHIPPenaltyDetailsConverter extends JsonUtils {
  ...
  lazy val lateSubmissionReads: Reads[LateSubmission] = (json: JsValue) => for {
    ...
    LateSubmission("lateSubmissionID", incomeSource, None, taxPeriodStartDate, taxPeriodEndDate, taxPeriodDueDate, returnReceiptDate, taxReturnStatus)
  }
  ...
  lazy val timeToPayReads: Reads[TimeToPay] = (
      (JsPath \ "TTPProposalDate").readNullable[LocalDate] and
      (JsPath \ "TTPAgreementDate").readNullable[LocalDate]
    )(TimeToPay.apply _)
