...
import uk.gov.hmrc.incometaxpenaltiesfrontend.models.compliance.SessionData
import uk.gov.hmrc.incometaxpenaltiesfrontend.utils.Logger.logger
import uk.gov.hmrc.incometaxpenaltiesfrontend.utils.PagerDutyHelper
import uk.gov.hmrc.incometaxpenaltiesfrontend.utils.PagerDutyHelper.PagerDutyKeys.{INVALID_JSON_RECEIVED_FROM_INCOME_TAX_SESSION_DATA, RECEIVED_4XX_FROM_INCOME_TAX_SESSION_DATA, RECEIVED_5XX_FROM_INCOME_TAX_SESSION_DATA}
    ...
    (_: String, _: String, response: HttpResponse) =>
      response.status match {
        case OK =>
          response.json.validate[SessionData] match {
            case JsSuccess(model, _) =>
              logger.debug(s"[GetSessionData][read] Successfully retrieved session data for user, response:\n${Json.toJson(model)}")
              Right(Some(model))
            case JsError(errors) =>
        ...
        case NOT_FOUND =>
          ...
          Right(None)
        case BAD_REQUEST =>
          logger.error(s"[GetSessionData][read]: Bad request returned with reason: ${response.body}")
