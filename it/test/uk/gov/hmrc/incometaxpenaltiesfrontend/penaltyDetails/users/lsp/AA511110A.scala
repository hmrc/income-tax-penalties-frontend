...
object AA511110A extends UserDetailsData {
  ...
  def penaltyCard1ExpectedContent(card: Element): Unit = {
    ...
    validateSummary(cardRows.get(1), "Pay penalty by", "22 December 2028")
    validateSummary(cardRows.get(2), "Update period", "6 July 2028 to 5 October 2028")
    validateSummary(cardRows.get(3), "Update due", "7 November 2028")
    validateSummary(cardRows.get(4), "Update submitted", "1 December 2028")
  ...
  def penaltyCard2ExpectedContent(card: Element): Unit = {
    ...
    validateSummary(cardRows.get(0), "Missing or late income sources", "JB Painting and Decorating UK property rental income")
    validateSummary(cardRows.get(1), "Update period", "6 April 2028 to 5 July 2028")
    validateSummary(cardRows.get(2), "Update due", "7 August 2028")
    validateSummary(cardRows.get(3), "Update submitted", "1 September 2028")
  ...
  def penaltyCard3ExpectedContent(card: Element): Unit = {
    ...
    cardRows.size() shouldBe 5
    validateSummary(cardRows.get(0), "Missing or late income sources", "JB Painting and Decorating UK property rental income")
    validateSummary(cardRows.get(1), "Update period", "6 January 2028 to 5 April 2028")
    validateSummary(cardRows.get(2), "Update due", "7 May 2028")
    validateSummary(cardRows.get(3), "Update submitted", "1 June 2028")
  ...
  override val expectedOverviewText: Boolean => String = isAgent =>
    s"Overview Your${if (isAgent) " client’s" else ""} account has: a late submission penalty the maximum number of late submission penalty points Check what you owe"
}
