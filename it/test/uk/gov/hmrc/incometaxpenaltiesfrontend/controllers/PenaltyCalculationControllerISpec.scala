      ...
      "render the expected first late payment calculation" when {
        "a first late payment penalty exists for the penaltyId" that {
          ...
          "is between 15 and 30 days and the tax is unpaid" in {
            ...
            document.getElementById("reasonList").getElementsByTag("li").get(0).text() shouldBe s"$youHaveOrThey missed the deadline by 15-30 days, so $youOrThey will be charged 3% of the tax that was outstanding 15 days after the payment deadline (£99.99)"
          ...
          "is between 15 and 30 days and the tax is paid but not penalty" in {
            ...
            document.getElementById("reasonList").getElementsByTag("li").get(0).text() shouldBe s"$youOrTheyCaps missed the deadline by 15-30 days, so $youOrThey have been charged 3% of the tax that was outstanding 15 days after the payment deadline (£99.99)"
      ...
      "render the expected second late payment calculation" when {
        "a second late payment penalty exists for the penaltyId" that {
          ...
          "tax is paid" in {
            ...
            document.getElementById("penaltyStatus").text() shouldBe s"To avoid interest charges, $youOrClient should pay this penalty by ${getDateString(secondLPPCalcData.payPenaltyBy)}."
